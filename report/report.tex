...
The tuning parameters are the number of trees and the proportion of outliers.
...
\paragraph{}
If $\textit{lof}_k(X)$ is larger than a threshold, then we classify $X$ is an outlier. Surely, the number of neighbors $k$ is a tuning parameter, and the proportion of outliers is also given by decision makers as a parameter. For more detailed description of the algorithm, readers could refer to \cite{lof}.
...
\par As for data splitting, we set the ratio among training, validation, and test set as 6:2:2. If the training set is small, say, less than half of the dataset, then training or modelling is difficult, especially for anomaly data. If the validation set or test set is too small, then our evaluation metric will be vulnerable to a tiny change in anomaly prediction. Our choice is a tradeoff.
